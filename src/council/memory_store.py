...
# Only inject historical triples that are sufficiently related to the request.
...
Verdict: {verdict[:1500]}..."""
    ...
    async def get_context(self, topic: str, extraction_model: str, top_k: int = 10) -> str:
        ...
        if not relevant:
            logger.info(
                ...
                extra={"best_score": round(scored[0][0], 4) if scored else None},
            )
            return ""
    ...
    def all_triples(self) -> list[Triple]:
        with self._connection() as conn:
            rows = conn.execute(
                "SELECT subject, predicate, object, confidence FROM memory_triples "
