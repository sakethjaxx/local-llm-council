...
#
# Default is 2, not 4: the hardware tiers deliberately put every seat on one
# shared local model so it stays resident in RAM, and firing four concurrent
# requests at that single Ollama model multiplies the KV cache and thrashes the
# exact memory budget the shared roster was chosen to protect.
MAX_PARALLEL_MEMBERS = max(1, int(os.getenv("COUNCIL_MAX_PARALLEL_MEMBERS", "2")))

# Sampling temperature. Local models default to ~0.8, which makes council seats
# ramble and disagree with themselves run to run; analysis wants determinism and
# the chairman's strict-JSON verdict wants near-zero.
...
# tiktoken/cl100k_base is only the native tokenizer for OpenAI models. For
# open-weight models (Qwen, Llama 3, Gemma, DeepSeek, Mistral) with differing
# vocab sizes (32k up to 256k) and byte fallback behaviors, we apply model-family
# calibrated scaling multipliers so token budgets do not overflow tight context windows.
...
class CouncilOrchestrator:
    ...
    async def _stream_llm_to_queue(
        ...
        for attempt in range(max_retries):
            ...
            try:
                # Hard wall-clock deadline. LiteLLM's own `timeout=` maps to an
                # httpx per-read timeout, so a local model that trickles bytes (or
                # sits queued behind other requests in Ollama) can blow far past it
                # — runs have been observed taking 900s+ against a 180s setting.
                # asyncio.timeout bounds the whole generation, no exceptions.
                acompletion_kwargs = {
                    "model": cfg["model"],
                    "messages": messages,
                ...
                if top_p_val is not None:
                    acompletion_kwargs["top_p"] = top_p_val

                async with asyncio.timeout(LLM_TIMEOUT_S):
                    resp = await litellm.acompletion(**acompletion_kwargs)
                ...
                # A wall-clock timeout means the model is too slow for this box, so
                # burning two more full-length attempts just multiplies the wait
                # (3 x 180s of dead time) without changing the outcome. Any partial
                # text already streamed is kept.
                ...
                if deadline_hit:
                    is_retryable, is_permanent = False, True
                else:
    ...
    async def run(
        ...
        try:
            ...
            async def _background_post_run(r_id: str, t_topic: str, v_verdict: str, m_model: str):
                # Yield a brief slice so client finishes receiving the SSE done event
                await asyncio.sleep(0.05)
                if is_shutdown_requested():
                    return
                try:
                    await memory_engine.extract_memory(t_topic, v_verdict, m_model, run_id=r_id)
                except Exception as b_exc:
                    logger.warning("background_memory_extraction_failed", extra={"run_id": r_id, "error": str(b_exc)})
                if is_shutdown_requested():
                    return
                try:
                    await skill_registry.extract_skills(r_id, t_topic, m_model)
                except Exception as b_exc:
    ...
    async def chat_with_member(
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                resp = await litellm.acompletion(**chat_kwargs)
                async for chunk in resp:
