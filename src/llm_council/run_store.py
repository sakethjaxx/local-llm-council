...
def _db_connect(path: str, check_same_thread: bool = True) -> sqlite3.Connection:
...
class StoredRun:
    ...
    council_confidence: Optional[int]
...
class RunStore:
    def __init__(self, db_path: str = DB_PATH):
        ...
        self._memory_conn = _db_connect(":memory:", check_same_thread=False) if db_path == ":memory:" else None
        self._init_db()
    ...
    def _init_db(self):
        try:
            with self._connection() as conn:
                ...
                self._apply_migration(conn, "004_confidence_metrics", "Add grounding, confidence, stance summary to runs", self._migrate_confidence_metrics)
    ...
    def _migrate_confidence_metrics(self, conn):
        cursor = conn.execute("PRAGMA table_info(runs)")
        cols = {row[1] for row in cursor.fetchall()}
        if "grounding_ratio" not in cols:
            conn.execute("ALTER TABLE runs ADD COLUMN grounding_ratio REAL")
        if "council_confidence" not in cols:
            conn.execute("ALTER TABLE runs ADD COLUMN council_confidence INTEGER")
        if "stance_summary" not in cols:
            conn.execute("ALTER TABLE runs ADD COLUMN stance_summary TEXT")
    ...
    def update_confidence_metrics(
        ...
        council_confidence: int | None = None,
        ...
        try:
            with self._connection() as conn:
                conn.execute(
                    """
                    UPDATE runs
                    SET grounding_ratio = COALESCE(?, grounding_ratio),
                        council_confidence = COALESCE(?, council_confidence),
                        stance_summary = COALESCE(?, stance_summary)
                    WHERE run_id = ?
                    """,
                    (grounding_ratio, council_confidence, stance_summary, run_id),
                )
        except Exception as exc:
            logger.exception("run_store_confidence_metrics_failed", extra={"run_id": run_id, "error": str(exc)})
    ...
    def list_quality_metrics(self, limit: int = 100) -> dict:
        ...
        with self._connection() as conn:
            rows = conn.execute(
                ...
                SELECT run_id, started_at, status, parse_tier, phase1_divergence,
                       specificity_score, smart_phase_score, grounding_ratio,
                       council_confidence
                FROM runs
        ...
        grounding_values = [
            float(row["grounding_ratio"])
            for row in completed
            if row.get("grounding_ratio") is not None
        ]
        confidence_values = [
            int(row["council_confidence"])
            for row in completed
            if row.get("council_confidence") is not None
        ]
        ...
        return {
            ...
            "summary": {
                ...
                "avg_grounding_ratio": (
                    sum(grounding_values) / len(grounding_values) if grounding_values else None
                ),
                "avg_council_confidence": (
    ...
    def _run_row_to_dict(self, row) -> dict:
        try:
            stance_summary = json.loads(row["stance_summary"]) if row["stance_summary"] else None
