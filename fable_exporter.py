"""Fable Graph Exporter for Local LLM Council.
Converts council memory triples into standard Fable Graph corpus JSON (.fable.json).
Complies with Fable Reasoning Protocol & Ponytail KISS principles.
"""

from typing import Any, Dict, List


def export_to_fable_json(
    triples: List[Dict[str, Any]],
    corpus_name: str = "local-llm-council",
    extracted_by: str = "local-llm-council:fable-engine"
) -> Dict[str, Any]:
    """Converts council memory triples into valid .fable.json corpus structure for
    ingestion into Fable Graph (node server/ingest.js corpus/<name>.fable.json).
    """
    ...
    for t in triples:
        ...
        # Normalize predicate to valid Fable edge vocabulary
        valid_types = {
        ...
        if subj_key not in notes_map:
            notes_map[subj_key] = {
                ...
                "extraction": {
                    ...
                    "tags": ["council-memory", "fable-triples"],
