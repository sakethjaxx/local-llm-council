...
class GraphExporterTests(unittest.TestCase):
    def test_export_to_graph_json(self):
        triples = [
        ...
        result = export_to_graph_json(triples, corpus_name="test-corpus")

        self.assertEqual(result["project"], "test-corpus")
        self.assertEqual(result["extracted_by"], "local-llm-council:memory-engine")
